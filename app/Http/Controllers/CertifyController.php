...
class CertifyController extends Controller
{
    ...
    public function eventCertify(Request $request, Event $event, EventPartisipant $participant)
    {
        ...
        $img->text('Telah berpatisipasi dalam acara', 1020, 775, function ($font) {
            $font->file('fonts/open-sans/OpenSans-Regular.ttf');
            $font->size(30);
            $font->align('center');
            $font->valign('top');
        });
        $img->text($event->title, 1020, 830, function ($font) {
            $font->file('fonts/open-sans/OpenSans-Bold.ttf');
            $font->size(30);
            $font->align('center');
            $font->valign('top');
        });
        $img->text('Malang, ' . Carbon::parse($participant->updated_at)->locale('id')->isoFormat('D MMMM YYYY'), 1020, 885, function ($font) {
            $font->file('fonts/open-sans/OpenSans-Regular.ttf');
            $font->size(30);
            $font->align('center');
            $font->valign('top');
    ...
    public function verification(
        ...
        $data['number'] = $number;
        $data['image'] = $src;
        return view('dashboard.user.pages.certify.verify', $data);
    }
}
