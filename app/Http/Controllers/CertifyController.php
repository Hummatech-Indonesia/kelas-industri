...
class CertifyController extends Controller
{
    ...
    public function certifyCompetenceTest(Classroom $classroom)
    {
        ...
        $text = '24' . $class . substr($classroom->generation->generation, 9) . Carbon::now()->locale('id')->format('ymd') . substr(auth()->user()->national_student_id, 6);
        $spacedText = implode('  ', str_split($text));
        $img->text($spacedText, 1045, 340, function ($font) {
            $font->file('fonts/poppins/Poppins-Regular.ttf');
            $font->size(42);
        ...
        ob_start();
        QrCode::size(100)->format('png')->generate(route('competence-verifycation', ['classroom' => $classroom, 'user' => auth()->user()->id]), 'php://output');
        $qrImage = ob_get_clean();
    ...
    public function eventVerification(
        ...
        $code = '24' . Carbon::now()->locale('id')->isoFormat('YYYYMMDD') . mt_rand(0, 9999);
        $spacedText = implode('  ', str_split($code));
        $img->text($spacedText, 1045, 340, function ($font) {
            $font->file('fonts/poppins/Poppins-Regular.ttf');
            $font->size(42);
    ...
    public function competenceTestVerify(Classroom $classroom, User $user)
    {
        $groupedScores = $this->certifyService->studentScore($classroom);
        ...
        $data = [
            'certificate' => base64_encode($img->toPng()),
            'score' => base64_encode($scoreImg->toPng()),
            'user' => $user,
            'number' => $code
        ];

        return view('dashboard.user.pages.certify.competenceTest', $data);
    }
}
